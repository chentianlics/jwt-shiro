...
public class HttpClientUtils {
	 ...
	 * @throws IOException
	 */
	public static String simplePostInvokeHeaders(String url,
	 ...
	 */
	public static HttpResponse postInvoke(String url,
			Map<String, String> params, Map<String, String> headers) throws URISyntaxException,ClientProtocolException, IOException{
		HttpClient client = buildHttpClient(false);
		// 创建post
		HttpPost postMethod = buildHttpPost(url, params);
		postMethod.setEntity(new StringEntity(JSONObject.toJSONString(params), ContentType.APPLICATION_JSON));
		...
		if (!headers.isEmpty()) {
			...
			postMethod.setHeaders(headers2);
		}
		return client.execute(postMethod);
	}
	...
	public static String simpleSSlKeyPostXMLInvoke(String url,
		...
		log.info("===========余额提现请求微信结果，result="+ JSON.toJSONString(response));
		HttpEntity entity = response.getEntity();
		if (entity != null) {
			return EntityUtils.toString(entity, UTF8_CHARSET);
		}
